...
public interface TransactionRepository extends JpaRepository<Transaction, Long>, CustomTransactionRepository {
    ...
    @Query(value = "SELECT DATE_TRUNC('day', T.created_at) as day, max(price) as highestPrice, min(price) lowestPrice FROM transaction T where trading_pair_id = :id AND T.created_at > :startTime AND T.created_at < :endTime GROUP BY day ORDER BY day", nativeQuery = true)
